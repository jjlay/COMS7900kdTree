...
void buildTree_parallel(double *data, int rows, int cols, Tree *tree) {
	//////////////////
	// the COMM_WORLD is also an argument, but I don't know how exactly to write that
	// also, myRank and numWorkers should be arguments
	//////////////////
	...
	// if tree.n > 1
		...
		// if numL > 0
			...
			// buildTree_parallel( data, xxx, cols, treeL, worldL )
			// tree.L = treeL
			// treeL.parent = tree
		// end
		// if numR > 0
			...
			// buildTree_parallel( data, xxx, cols, treeR, worldR )
			// tree.R = treeR
			// treeR.parent = tree
		// end
	// else
