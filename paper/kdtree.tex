...
% farzi
STATEMENT OF THE PROBLEM: talk about orthogonal recursive bisection ORB, needed to search all  data with 3 radii and 501 file centers, return table of counts
...
Bad
\begin{itemize}
	\item parallelSort conversions
	\item memory leaks
	\item multiple communicators (comm)
	\item adaptBins not converging
	\item cout clutter debug
	\item out of bounds issues
	\item *data[], *rows
	\item didn't plan ahead what should be passed into functions, required constant editing of h files
\end{itemize}

Good
\begin{itemize}
	\item few merge conflicts
	\item fast debugging through extreme coding
	\item visualizing output (validation)
	\item efficient delegation that avoided merge conflicts
\end{itemize}
...
\begin{itemize}
	\item cloud
	\item use this code for personal research
\end{itemize}
